...
for($day = time(); date('Y-m-d',$day_end) > date('Y-m-d',$day); $day += (60*60*24))
{
		...
		$info[$date]['sunrise'] = date('g:i',(60*(round(date_sunrise($day,SUNFUNCS_RET_TIMESTAMP,35.11666,-89.87740,91)/(60)))));
		if (date('D',$day) == 'Fri') {
			$timestamp	= $day;
			...
			$info[$date]['shacharis'] = '6:35, 8:00';
			//get sunset of next week, and see which sunset is earlier
			$min_sunset = min($shkiah_timestamp,strtotime('-5 days',get_sunset(strtotime('+5 days',$timestamp))));
			...
			$info[$date]['shacharis'] = date('D',$day) == 'Sun' ? '8:00' : '6:35, 8:00';
		} 
}
