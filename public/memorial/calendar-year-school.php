...
$long = $_GET['long'] ? -1*$_GET['long'] : -89.87740;

header('Content-type: text/html; charset=utf-8'); 
//first set the timezone that PHP will use
date_default_timezone_set(get_nearest_timezone($lat,$long));
//echo 'https://clients6.google.com/calendar/v3/calendars/jv8f8iv272av77cnpj14e6efh4@group.calendar.google.com/events?calendarId=jv8f8iv272av77cnpj14e6efh4@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+400 days"));
//die(file_get_contents('https://clients6.google.com/calendar/v3/calendars/mhafyos.org_csuae1um74mggl43lh9p6ip6rs@group.calendar.google.com/events?calendarId=mhafyos.org_csuae1um74mggl43lh9p6ip6rs@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+400 days"))));
$cal = json_decode(file_get_contents('https://clients6.google.com/calendar/v3/calendars/mhafyos.org_csuae1um74mggl43lh9p6ip6rs@group.calendar.google.com/events?calendarId=mhafyos.org_csuae1um74mggl43lh9p6ip6rs@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+600 days"))));
//$ptaevents = json_decode(file_get_contents('https://clients6.google.com/calendar/v3/calendars/m2ngfo8siap8tujdjh9im064m8@group.calendar.google.com/events?calendarId=m2ngfo8siap8tujdjh9im064m8@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+400 days"))));
$annivs = json_decode(file_get_contents('https://clients6.google.com/calendar/v3/calendars/5001652c121cfe719bdb12fe2e5ec0059e8d901f1b0eee4c7038663ad7203818@group.calendar.google.com/events?calendarId=5001652c121cfe719bdb12fe2e5ec0059e8d901f1b0eee4c7038663ad7203818@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+400 days"))));
$birthdays = json_decode(file_get_contents('https://clients6.google.com/calendar/v3/calendars/831a73b7b8fbe15647e52654acf310e7a5cfc0dbe05925fb935b5846928896ff@group.calendar.google.com/events?calendarId=831a73b7b8fbe15647e52654acf310e7a5cfc0dbe05925fb935b5846928896ff@group.calendar.google.com&singleEvents=true&timeZone=America%2FChicago&maxAttendees=1&maxResults=500&sanitizeHtml=true&key=AIzaSyBNlYH01_9Hc5S1J9vuFmu2nUqBZJNAXxs&orderBy=starttime&timeMin='.date("Y-m-d\TH:i:s\Z", strtotime("today")).'&timeMax='.date("Y-m-d\TH:i:s\Z", strtotime("+400 days"))));
// print_r($birthdays);

//https://www.iconsdb.com/icons/preview/red/balloon-6-xl.png

//$number_of_days_to_show = 45;
//$day_end = ($number_of_days_to_show*60*60*24)+time(); 
$json_this_month = file_get_contents("hebcal.json"); //http://www.hebcal.com/hebcal/?v=1&cfg=json&nh=on&nx=on&year=$cal_year&month=".date('m',strtotime($cal_month." 1 ".$cal_year))."&ss=on&mf=on&s=on&i=off&lg=s&F=on");
...
<?php
foreach($cal->items as $school_event) {
	if ($school_event->start->dateTime) { 
		$event_date = date('Y-m-d',strtotime($school_event->start->dateTime));
		$info[$event_date]['school_event'] = $school_event->summary.': '.date('g:i',strtotime($school_event->start->dateTime));;
	} else {
		$start_date = $school_event->start->date;
		$info[$start_date]['school_event'] = $school_event->summary;
		$end_date = date('Y-m-d',strtotime('-1 day',strtotime($school_event->end->date)));
		//echo $start_date.'-'.$end_date.' ';
		while($start_date != $end_date)
		{
			$start_date = date('Y-m-d',strtotime('+1 day',strtotime($start_date)));
			if (date('D',strtotime($start_date)) != 'Sat' && date('D',strtotime($start_date)) != 'Sun') {
				$info[$start_date]['school_event'] = $school_event->summary;
			}
		}
	}
}
foreach($birthdays->items as $birthday) {
	$start_date = $birthday->start->date;
	if (isset($info[$start_date]['birthday'])) {$info[$start_date]['birthday'] .= '<br/>'.$birthday->summary; }
	else {
		$info[$start_date]['birthday'] = $birthday->summary;
	}
}

foreach($annivs->items as $anniv) {
	$start_date = $anniv->start->date;
	if (isset($info[$start_date]['anniv'])) {$info[$start_date]['anniv'] .= '<br/>'.$anniv->summary; }
	else {
		$info[$start_date]['anniv'] = $anniv->summary;
	}
}

foreach($ptaevents->items as $ptaevent) {
	$start_date = $ptaevent->start->date;
	if (isset($info[$start_date]['ptaevent'])) {$info[$start_date]['ptaevent'] .= '<br/>'.$ptaevent->summary; }
	else {
		$info[$start_date]['ptaevent'] = $ptaevent->summary;
	}
}
	...
	if ($event->category == 'holiday') {
		...
		if($event->title == "Erev Purim") {
			...
			if (date('D',strtotime($event->date)) == 'Sat') {
				$info[$event->date]['megilla1']= date('g:i',(5*60)*(ceil(strtotime('+60 minutes',$sunset)/(5*60))));
			}
		}
...
$save_info = $info;
$months = array('Aug'=>$cal_year,'Sept'=>$cal_year,'October'=>$cal_year,'November'=>$cal_year,'December'=>$cal_year,'January'=>$cal_year+1,'February'=>$cal_year+1,'March'=>$cal_year+1,'April'=>$cal_year+1,'May'=>$cal_year+1,'June'=>$cal_year+1,'July'=>$cal_year+1,'August'=>$cal_year+1,'September'=>$cal_year+1);
...
//print_r($cal);
	...
	for($day = strtotime($cal_month." 1 ".$cal_year.' 9 AM'); date('Y-m-d',$day) <= $last_day_in_month; $day += (60*60*24))
	{
			...
			if (date('D',$day) == 'Fri') {
				...
				$mincha_this_week = (5*60)*(round((strtotime('-12 minutes',$min_sunset)/(5*60))));
				if ($mincha_this_week > ($min_sunset - (60*12))) { // if there is less than a 12 minute gap, add five minutes
				...
				$min_sunset = min(strtotime('+1 day',get_sunset(strtotime('-1 day',$timestamp))),strtotime('+5 days',get_sunset(strtotime('-5 days',$timestamp))));
				// then subtract 12 minutes from the earlier one, and round to the nearest 5 minute mark
				$mincha_last_week = (5*60)*(round((strtotime('-12 minutes',$min_sunset)/(5*60))));
				//echo $mincha_last_week.' '.($min_sunset-600) . ' | ';
				//echo date('g:i',$mincha_last_week).' '.date('g:i',$min_sunset).' | ';
				if ($mincha_last_week > ($min_sunset - (60*12))) { // if there is less than a 12 minute gap, add five minutes
...
// foreach($months as $cal_month => $cal_year) {
	...
	<table style="width:1018px;"><tr><td style="font-size:25px;margin-bottom:0px;page-break-before: always;"><?php echo $cal_month.' '.$cal_year;?></td>
	<td style="font-size:32px;margin-bottom:0px;direction:rtl;"><?php echo iconv ('WINDOWS-1255', 'UTF-8', jdtojewish(unixtojd(strtotime($cal_month.' 1, '.$cal_year)),true,CAL_JEWISH_ADD_GERESHAYIM));?> - <?php echo iconv ('WINDOWS-1255', 'UTF-8', jdtojewish(unixtojd(strtotime(date("Y-m-t", strtotime($cal_month.' 1, '.$cal_year)))-(60*60*24)),true,CAL_JEWISH_ADD_GERESHAYIM));?></td>
	</tr></table>
	<table id="main_table">
	...
	foreach($info as $date => $info_day) {
		// if ($date >= date('Y-m-d')) {
			...
			echo $info_day['school_event'] ? '<strong>'.$info_day['school_event'].'</strong><br/>' : '';
			...
			// echo $info_day['burn_by'] ? 'Burn Chametz By: '. $info_day['burn_by'].'<br/>' : '';
			...
			echo $info_day['candlelighting'] ? ($info_day['candlelighting'] != $info_day['mincha'] ? 'Candles: ' : 'Candles: '). $info_day['candlelighting'].'<br/>' : '';
			...
			echo $info_day['ptaevent'] ? '<span style="color:blue;">'.$info_day['ptaevent'].'</span><br/>' : '';

			echo $info_day['birthday'] ? '<img style="width: 18px;	position: relative; top: 4px;" src="balloon-2-xl.png"/><span style="color:red;">'.$info_day['birthday'].'</span><br/>' : '';
			echo $info_day['anniv'] ? '<img style="width: 18px;	position: relative; top: 4px;" src="rings.png"/><span style="color:silver;">'.$info_day['anniv'].'</span><br/>' : '';
...
{
	return (60*(round(date_sunset($timestamp,SUNFUNCS_RET_TIMESTAMP,$GLOBALS['lat'],$GLOBALS['long'],90+(3.5/6))/(60))))+$offset;
}

function get_sunrise($timestamp,$offset = 0)
{
	return (date_sunrise($timestamp,SUNFUNCS_RET_TIMESTAMP,$GLOBALS['lat'],$GLOBALS['long'],90.6))+$offset;
}
